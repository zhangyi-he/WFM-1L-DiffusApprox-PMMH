...
// [[Rcpp::export]]
arma::dmat simulateWFD_arma(const double& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const arma::dcolvec& int_frq, const int& int_gen, const int& lst_gen, const arma::uword& ptn_num, const arma::uword& pth_num) {
  // ensure RNG gets set/reset
  RNGScope scope;
  ...
  // declare delta W
  arma::dmat dW = pow(dt, 0.5) * arma::randn<arma::dmat>(pth_num, arma::uword(lst_gen - int_gen) * ptn_num);

  // declare the mutant allele frequency trajectory
  arma::dmat mut_frq_pth = arma::zeros<arma::dmat>(pth_num, arma::uword(lst_gen - int_gen) * ptn_num + 1);

  // initialise the mutant allele frequency in generation 0
  mut_frq_pth.col(0) = int_frq;
  ...
  for (arma::uword t = 1; t < arma::uword(lst_gen - int_gen) * ptn_num + 1; t++) {
    ...
    arma::dcolvec mu = scl_sel_cof * mut_frq_pth.col(t - 1) % (1 - mut_frq_pth.col(t - 1)) % (dom_par + (1 - 2 * dom_par) * mut_frq_pth.col(t - 1));

    // calculate the diffusion coefficient
    arma::dcolvec sigma = pow(mut_frq_pth.col(t - 1) % (1 - mut_frq_pth.col(t - 1)) / siz_rto(t - 1), 0.5);

    // proceed the Euler-Maruyama scheme
    arma::dcolvec mut_frq = mut_frq_pth.col(t - 1) + mu * dt + sigma % dW.col(t - 1);
    if (any(mut_frq < 0)) {
      arma::ucolvec idx = arma::find(mut_frq < 0);
      mut_frq.elem(idx) = arma::zeros<arma::dcolvec>(idx.n_elem);
    }
    if (any(mut_frq > 1)) {
      arma::ucolvec idx = arma::find(mut_frq > 1);
      mut_frq.elem(idx) = arma::ones<arma::dcolvec>(idx.n_elem);
    }
    mut_frq_pth.col(t) = mut_frq;
  }
...
List runBPF_arma(const arma::dcolvec& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const int& evt_gen, const arma::dmat& raw_smp, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  for (arma::uword k = 1; k < evt_idx; k++) {
    ...
    arma::dmat path = simulateWFD_arma(sel_cof(0), dom_par, pop_siz.subvec(smp_gen(k - 1), smp_gen(k)), ref_siz, mut_frq_tmp, smp_gen(k - 1), smp_gen(k), ptn_num, pcl_num);
    mut_frq_pth.cols((smp_gen(k - 1) - smp_gen(0)) * ptn_num, (smp_gen(k) - smp_gen(0)) * ptn_num) = path;
    mut_frq_tmp = arma::conv_to<arma::dcolvec >::from(path.tail_cols(1));
    gen_frq_tmp = calculateGenoFrq_arma(sel_cof(0), dom_par, mut_frq_tmp);
  ...
  if (evt_idx < smp_gen.n_elem) {
    if (smp_gen(evt_idx - 1) != evt_gen) {
      ...
      arma::dmat path1 = simulateWFD_arma(sel_cof(0), dom_par, pop_siz.subvec(smp_gen(evt_idx - 1), evt_gen), ref_siz, mut_frq_tmp, smp_gen(evt_idx - 1), evt_gen, ptn_num, pcl_num);
      mut_frq_pth.cols((smp_gen(evt_idx - 1) - smp_gen(0)) * ptn_num, (evt_gen - smp_gen(0)) * ptn_num) = path1;
      mut_frq_tmp = arma::conv_to<arma::dcolvec >::from(path1.tail_cols(1));

      cout << "generation: " << smp_gen(evt_idx) << endl;
      wght_tmp = arma::zeros<arma::dcolvec>(pcl_num);
      arma::dmat path2 = simulateWFD_arma(sel_cof(1), dom_par, pop_siz.subvec(evt_gen, smp_gen(evt_idx)), ref_siz, mut_frq_tmp, evt_gen, smp_gen(evt_idx), ptn_num, pcl_num);
      mut_frq_pth.cols((evt_gen - smp_gen(0)) * ptn_num, (smp_gen(evt_idx) - smp_gen(0)) * ptn_num) = path2;
      mut_frq_tmp = arma::conv_to<arma::dcolvec >::from(path2.tail_cols(1));
      ...
      wght_tmp = arma::zeros<arma::dcolvec>(pcl_num);
      mut_frq_tmp = mut_frq_pst.col(evt_idx - 1);
      arma::dmat path = simulateWFD_arma(sel_cof(1), dom_par, pop_siz.subvec(smp_gen(evt_idx - 1), smp_gen(evt_idx)), ref_siz, mut_frq_tmp, smp_gen(evt_idx - 1), smp_gen(evt_idx), ptn_num, pcl_num);
      mut_frq_pth.cols((smp_gen(evt_idx - 1) - smp_gen(0)) * ptn_num, (smp_gen(evt_idx) - smp_gen(0)) * ptn_num) = path;
      mut_frq_tmp = arma::conv_to<arma::dcolvec >::from(path.tail_cols(1));
      gen_frq_tmp = calculateGenoFrq_arma(sel_cof(1), dom_par, mut_frq_tmp);
      wght_tmp = calculateParticleWeight_arma(gen_lik(evt_idx), sel_cof(1), dom_par, mut_frq_tmp);
    ...
    for (arma::uword k = evt_idx + 1; k < smp_gen.n_elem; k++) {
      ...
      wght_tmp = arma::zeros<arma::dcolvec>(pcl_num);
      mut_frq_tmp = mut_frq_pst.col(k - 1);
      arma::dmat path = simulateWFD_arma(sel_cof(1), dom_par, pop_siz.subvec(smp_gen(k - 1), smp_gen(k)), ref_siz, mut_frq_tmp, smp_gen(k - 1), smp_gen(k), ptn_num, pcl_num);
      mut_frq_pth.cols((smp_gen(k - 1) - smp_gen(0)) * ptn_num, (smp_gen(k) - smp_gen(0)) * ptn_num) = path;
      mut_frq_tmp = arma::conv_to<arma::dcolvec >::from(path.tail_cols(1));
      gen_frq_tmp = calculateGenoFrq_arma(sel_cof(1), dom_par, mut_frq_tmp);
...
void calculateLogLikelihood_arma(double& log_lik, arma::drowvec& frq_pth, const arma::dcolvec& sel_cof, const double& dom_par, const arma::icolvec& pop_siz, const int& ref_siz, const int& evt_gen, const arma::irowvec& smp_gen, const arma::field<arma::dmat>& gen_lik, const arma::uword& ptn_num, const arma::uword& pcl_num) {
  ...
  arma::dmat mut_frq_pth = arma::zeros<arma::dmat>(pcl_num, arma::uword(arma::max(smp_gen) - arma::min(smp_gen)) * ptn_num + 1);
  arma::dcolvec mut_frq = arma::zeros<arma::dcolvec>(pcl_num);

  // before the event of interest
  // initialise the particles
  mut_frq = arma::randu<arma::dcolvec>(pcl_num);
  wght = calculateParticleWeight_arma(gen_lik(0), sel_cof(0), dom_par, mut_frq);
  ...
  // run the bootstrap particle filter
  for (arma::uword k = 1; k < evt_idx; k++) {
    arma::dmat path = simulateWFD_arma(sel_cof(0), dom_par, pop_siz.subvec(smp_gen(k - 1), smp_gen(k)), ref_siz, mut_frq, smp_gen(k - 1), smp_gen(k), ptn_num, pcl_num);
    mut_frq_pth.cols((smp_gen(k - 1) - smp_gen(0)) * ptn_num, (smp_gen(k) - smp_gen(0)) * ptn_num) = path;
    mut_frq = arma::conv_to<arma::dcolvec >::from(path.tail_cols(1));
    wght = calculateParticleWeight_arma(gen_lik(k), sel_cof(0), dom_par, mut_frq);
  ...
  if (evt_idx < smp_gen.n_elem) {
    if (smp_gen(evt_idx - 1) != evt_gen) {
      arma::dmat path1 = simulateWFD_arma(sel_cof(0), dom_par, pop_siz.subvec(smp_gen(evt_idx - 1), evt_gen), ref_siz, mut_frq, smp_gen(evt_idx - 1), evt_gen, ptn_num, pcl_num);
      mut_frq_pth.cols((smp_gen(evt_idx - 1) - smp_gen(0)) * ptn_num, (evt_gen - smp_gen(0)) * ptn_num) = path1;
      mut_frq = arma::conv_to<arma::dcolvec >::from(path1.tail_cols(1));

      arma::dmat path2 = simulateWFD_arma(sel_cof(1), dom_par, pop_siz.subvec(evt_gen, smp_gen(evt_idx)), ref_siz, mut_frq, evt_gen, smp_gen(evt_idx), ptn_num, pcl_num);
      mut_frq_pth.cols((evt_gen - smp_gen(0)) * ptn_num, (smp_gen(evt_idx) - smp_gen(0)) * ptn_num) = path2;
      mut_frq = arma::conv_to<arma::dcolvec >::from(path2.tail_cols(1));
      wght = calculateParticleWeight_arma(gen_lik(evt_idx), sel_cof(1), dom_par, mut_frq);
      ...
      arma::dmat path = simulateWFD_arma(sel_cof(1), dom_par, pop_siz.subvec(smp_gen(evt_idx - 1), smp_gen(evt_idx)), ref_siz, mut_frq, smp_gen(evt_idx - 1), smp_gen(evt_idx), ptn_num, pcl_num);
      mut_frq_pth.cols((smp_gen(evt_idx - 1) - smp_gen(0)) * ptn_num, (smp_gen(evt_idx) - smp_gen(0)) * ptn_num) = path;
      mut_frq = arma::conv_to<arma::dcolvec >::from(path.tail_cols(1));
      wght = calculateParticleWeight_arma(gen_lik(evt_idx), sel_cof(1), dom_par, mut_frq);
    ...
    for (arma::uword k = evt_idx + 1; k < smp_gen.n_elem; k++) {
      arma::dmat path = simulateWFD_arma(sel_cof(1), dom_par, pop_siz.subvec(smp_gen(k - 1), smp_gen(k)), ref_siz, mut_frq, smp_gen(k - 1), smp_gen(k), ptn_num, pcl_num);
      mut_frq_pth.cols((smp_gen(k - 1) - smp_gen(0)) * ptn_num, (smp_gen(k) - smp_gen(0)) * ptn_num) = path;
      mut_frq = arma::conv_to<arma::dcolvec >::from(path.tail_cols(1));
      wght = calculateParticleWeight_arma(gen_lik(k), sel_cof(1), dom_par, mut_frq);
