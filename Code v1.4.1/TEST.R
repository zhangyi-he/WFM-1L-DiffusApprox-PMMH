...
frq_pth <- cmpsimulateWFD(sel_cof, dom_par, pop_siz, ref_siz, int_frq, evt_gen, int_gen, lst_gen, ptn_num, pth_num, dat_aug = TRUE)
...
for (i in 1:sim_num) {
  ...
  smp_WFD[i] <- cmpsimulateWFD(sel_cof, dom_par, pop_siz, ref_siz, int_frq, evt_gen, int_gen, lst_gen, ptn_num, pth_num = 1, dat_aug = FALSE)[(lst_gen - int_gen) + 1]
}
