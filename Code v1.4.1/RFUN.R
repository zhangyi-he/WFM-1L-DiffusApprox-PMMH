...
# call C++ functions
sourceCpp("./Code v1.4.1/CFUN.cpp")
...
#' Standard version
simulateWFD <- function(sel_cof, dom_par, pop_siz, ref_siz, int_frq, evt_gen, int_gen, lst_gen, ptn_num, pth_num, dat_aug = TRUE) {
  if (length(int_frq) == pth_num) {
    if (evt_gen >= lst_gen) {
      mut_frq_pth <- simulateWFD_arma(sel_cof[1], dom_par, pop_siz, ref_siz, int_frq, int_gen, lst_gen, ptn_num, pth_num)
      mut_frq_pth <- as.matrix(mut_frq_pth)
    } else if (evt_gen < int_gen) {
      mut_frq_pth <- simulateWFD_arma(sel_cof[2], dom_par, pop_siz, ref_siz, int_frq, int_gen, lst_gen, ptn_num, pth_num)
      mut_frq_pth <- as.matrix(mut_frq_pth)
    } else {
      mut_frq_pth_pre_evt <- simulateWFD_arma(sel_cof[1], dom_par, pop_siz[1:(evt_gen - int_gen + 1)], ref_siz, int_frq, int_gen, evt_gen, ptn_num, pth_num)
      mut_frq_pth_pre_evt <- as.matrix(mut_frq_pth_pre_evt)

      mut_frq_pth_pst_evt <- simulateWFD_arma(sel_cof[2], dom_par, pop_siz[(evt_gen - int_gen + 1):(lst_gen - int_gen + 1)], ref_siz, mut_frq_pth_pre_evt[, ncol(mut_frq_pth_pre_evt)], evt_gen, lst_gen, ptn_num, pth_num)
      mut_frq_pth_pst_evt <- as.matrix(mut_frq_pth_pst_evt)

      if (dim(mut_frq_pth_pre_evt)[1] > 1) {
        mut_frq_pth <- cbind(mut_frq_pth_pre_evt, mut_frq_pth_pst_evt[, -1])
      } else {
        mut_frq_pth <- t(as.matrix(append(mut_frq_pth_pre_evt, mut_frq_pth_pst_evt[, -1])))
      }
    }
  } else {
    if (evt_gen >= lst_gen) {
      mut_frq_pth <- simulateWFD_arma(sel_cof[1], dom_par, pop_siz, ref_siz, rep(int_frq, length.out = pth_num), int_gen, lst_gen, ptn_num, pth_num)
      mut_frq_pth <- as.matrix(mut_frq_pth)
    } else if (evt_gen < int_gen) {
      mut_frq_pth <- simulateWFD_arma(sel_cof[2], dom_par, pop_siz, ref_siz, rep(int_frq, length.out = pth_num), int_gen, lst_gen, ptn_num, pth_num)
      mut_frq_pth <- as.matrix(mut_frq_pth)
    } else {
      mut_frq_pth_pre_evt <- simulateWFD_arma(sel_cof[1], dom_par, pop_siz[1:(evt_gen - int_gen + 1)], ref_siz, rep(int_frq, length.out = pth_num), int_gen, evt_gen, ptn_num, pth_num)
      mut_frq_pth_pre_evt <- as.matrix(mut_frq_pth_pre_evt)
  ...
  if (dat_aug == FALSE) {
    if (dim(mut_frq_pth)[1] > 1) {
      return(mut_frq_pth[, (0:(lst_gen - int_gen)) * ptn_num + 1])
    } else {
      return(t(as.matrix(mut_frq_pth[, (0:(lst_gen - int_gen)) * ptn_num + 1])))
    }
  } else {
    return(mut_frq_pth)
  }
...
simulateHMM <- function(model, sel_cof, dom_par, pop_siz, int_con, evt_gen, smp_lab, smp_gen, smp_qua, thr_val, ...) {
  ...
  if (model == "WFD") {
    mut_frq <- cmpsimulateWFD(sel_cof, dom_par, pop_siz, ref_siz, int_con, evt_gen, int_gen, lst_gen, ptn_num, pth_num = 1, dat_aug = FALSE)
    mut_frq <- as.vector(mut_frq[1, ])
...
runBayesianProcedure <- function(sel_cof, dom_par, pop_siz, ref_siz, evt_gen, raw_smp, ptn_num, pcl_num, itn_num, brn_num, thn_num, adp_set, ...) {
  ...
  # MMSE estimates for population mutant frequency trajectory with their 95% HPD intervals
